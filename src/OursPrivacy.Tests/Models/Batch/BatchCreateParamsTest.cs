...
public class BatchCreateParamsTest : TestBase
{
    ...
    public void Url_Works()
    {
        ...
        Assert.True(TestBase.UrisEqual(new Uri("https://api.oursprivacy.com/api/v1/batch"), url));
    }
...
public class EventTest : TestBase
{
    ...
    public void FieldRoundtrip_Works()
    {
        ...
        foreach (var item in expectedEventProperties)
        {
            Assert.True(model.EventProperties.TryGetValue(item.Key, out var value));

            Assert.Equal(value, model.EventProperties[item.Key]);
        }
        Assert.Equal(expectedExternalID, model.ExternalID);
        Assert.Equal(expectedIdentityContext, model.IdentityContext);
        Assert.Equal(expectedTime, model.Time);
        Assert.Equal(expectedUserID, model.UserID);
        Assert.Equal(expectedUserProperties, model.UserProperties);
    }
    ...
    public void FieldRoundtripThroughSerialization_Works()
    {
        ...
        foreach (var item in expectedEventProperties)
        {
            Assert.True(deserialized.EventProperties.TryGetValue(item.Key, out var value));

            Assert.Equal(value, deserialized.EventProperties[item.Key]);
        }
        Assert.Equal(expectedExternalID, deserialized.ExternalID);
        Assert.Equal(expectedIdentityContext, deserialized.IdentityContext);
        Assert.Equal(expectedTime, deserialized.Time);
        Assert.Equal(expectedUserID, deserialized.UserID);
        Assert.Equal(expectedUserProperties, deserialized.UserProperties);
    }
    ...
    public void OptionalNullablePropertiesUnsetAreNotSet_Works()
    {
        ...
        Assert.False(model.RawData.ContainsKey("eventProperties"));
        Assert.Null(model.ExternalID);
        Assert.False(model.RawData.ContainsKey("externalId"));
        Assert.Null(model.IdentityContext);
        Assert.False(model.RawData.ContainsKey("identityContext"));
        ...
        Assert.Null(model.UserProperties);
        Assert.False(model.RawData.ContainsKey("userProperties"));
    }
    ...
    public void OptionalNullablePropertiesSetToNullAreSetToNull_Works()
    {
        ...
        Assert.Null(model.EventProperties);
        Assert.True(model.RawData.ContainsKey("eventProperties"));
        Assert.Null(model.ExternalID);
        Assert.True(model.RawData.ContainsKey("externalId"));
        Assert.Null(model.IdentityContext);
        Assert.True(model.RawData.ContainsKey("identityContext"));
        Assert.Null(model.Time);
        Assert.True(model.RawData.ContainsKey("time"));
        Assert.Null(model.UserID);
        Assert.True(model.RawData.ContainsKey("userId"));
        Assert.Null(model.UserProperties);
        Assert.True(model.RawData.ContainsKey("userProperties"));
    }
