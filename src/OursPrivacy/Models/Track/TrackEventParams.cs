...
/// email so the event can be associated with a visitor. Identity resolution runs
/// in priority order: userId (direct, no lookup) → externalId (lookup by your ID)
/// → email (fallback lookup). If you know both userId and externalId, send both.
/// For top-level visitor properties: null clears the existing value, while undefined,
/// omitted fields, and empty strings are ignored. For entries inside custom_properties:
/// null, undefined, and empty strings are all ignored (custom_properties use merge
/// semantics). See https://docs.oursprivacy.com/docs/data-types for details and common pitfalls.
///
/// <para>NOTE: Do not inherit from this type outside the SDK unless you're okay with
/// breaking changes in non-major versions. We may add new methods in the future that
/// cause existing derived classes to break.</para>
/// </summary>
public record class TrackEventParams : ParamsBase
{
    ...
    /// — if omitted, Ours will generate one for you, but supplying your own gives
    /// you stronger idempotency guarantees (e.g. a Stripe payment intent ID or your
    /// internal order ID).
    /// </summary>
    public string? DistinctID
    {
        ...
        {
            this._rawBodyData.Freeze();
            return this._rawBodyData.GetNullableClass<string>("distinctId");
        }
        init { this._rawBodyData.Set("distinctId", value); }
    }
    ...
    /// </summary>
    public IReadOnlyDictionary<string, string?>? EventProperties
    {
        get
        {
            this._rawBodyData.Freeze();
    ...
    public double? Time
    {
        ...
        {
            this._rawBodyData.Freeze();
            return this._rawBodyData.GetNullableStruct<double>("time");
        }
        init { this._rawBodyData.Set("time", value); }
    }
    ...
    /// <summary>
    /// Properties to set on the visitor. (optional) You can also update these properties
    ...
    public UserProperties? UserProperties
    {
        ...
        {
            this._rawBodyData.Freeze();
            return this._rawBodyData.GetNullableClass<UserProperties>("userProperties");
        }
        init { this._rawBodyData.Set("userProperties", value); }
    }

    public TrackEventParams() { }

#pragma warning disable CS8618
    [SetsRequiredMembers]
    public TrackEventParams(TrackEventParams trackEventParams)
        : base(trackEventParams)
    {
        this._rawBodyData = new(trackEventParams._rawBodyData);
    }
...
#pragma warning restore CS8618
    ...
    public static TrackEventParams FromRawUnchecked(
        IReadOnlyDictionary<string, JsonElement> rawHeaderData,
        IReadOnlyDictionary<string, JsonElement> rawQueryData,
        IReadOnlyDictionary<string, JsonElement> rawBodyData
    )
    ...
    public virtual bool Equals(TrackEventParams? other)
    {
        if (other == null)
        {
            return false;
    ...
    {
        return new UriBuilder(options.BaseUrl.ToString().TrimEnd('/') + "/track")
        {
