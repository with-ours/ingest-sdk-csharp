...
public record struct ClientOptions()
{
    ...
    Lazy<string> _baseUrl = new(() =>
        Environment.GetEnvironmentVariable("OURS_PRIVACY_BASE_URL") ?? EnvironmentUrl.Production
    );
    ...
    /// <see cref="IOursPrivacyClient.WithRawResponse"/>, don't perform validation until deserialization
    /// is triggered.</para>
    /// </summary>
